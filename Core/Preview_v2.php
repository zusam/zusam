...
function preview_getBaseURL(&$p) {
	...
	$url = $p['url'];
	//$base_url = preg_replace("/.*(http:\/\/[^\/\?]+).*/","$1",$url);
...
function preview_getTitle(&$p) {
	...
	if($title != null) {
		...
		//$title = utf8_encode(to_utf8($p['html']('title')[0]->getPlainText()));
		$tmp = $p['html']('title');
		if(count($tmp) > 0) {
...
function preview_getDescription(&$p) {
	...
	if($description != null) {
		...
		//$description = to_utf8($description[0]->content);
...
function ranger($url) {
	//only jpg and gif (no png nor bmp)
	$t = microtime(true);
